...
public class AWSServerCommand implements Serializable {
    ...
    private static String serverAddress = "192.168.2.7";
    ...
    public int createParty(String partyName, String hostName, double latitude, double longitude)
    {
        this.partyName = partyName;
        this.hostName = hostName;
        try {
                               ...
                               ArrayList<String> hostNames, ArrayList<Party> matchedParties)
    {
        try {
            ...
            while(!aPartyID.equals(endConnect))
            {
                ...
                matchedParties.add(new Party(Integer.parseInt(aPartyID), aPartyName, aHostname, latitude, longitude));
    ...
    public boolean getPlaylistHost(int partyID, ArrayList<SongData> playList)
    {
        try {
            ...
            os.write(GET_SONGS);    //Notify server that a party needs songs
            //RegisterPartyActivity registerPartyActivity = new RegisterPartyActivity();
            //double []location = registerPartyActivity.getLocation();
            //Log.d("Lat", location[0]+"");
            //Log.d("Long", location[1]+"");
